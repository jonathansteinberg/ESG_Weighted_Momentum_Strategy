...
esg_momentum_construction <- function(J,S,K){
...
for (i in c(3,6,9,12)){
	for (j in c(1,3,6,9,12)){
		losers <- cbind(losers,esg_momentum_construction(i,0,j)[,1])
		winners <- cbind(winners,esg_momentum_construction(i,0,j)[,10])
		WMLs <- cbind(WMLs,esg_momentum_construction(i,0,j)[,11])}}
...
print(winners)
print(WMLs)
write.csv(losers,"/Users/jonathansteinberg/Desktop/R Group Project/ESG_LOSERS.csv", row.names = TRUE)
write.csv(winners,"/Users/jonathansteinberg/Desktop/R Group Project/ESG_WINNERS.csv", row.names = TRUE)
write.csv(WMLs,"/Users/jonathansteinberg/Desktop/R Group Project/ESG_WMLs.csv", row.names = TRUE)
